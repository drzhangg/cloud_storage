...
func CompleteUploadHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		w.Write(util.NewRespMsg(-1, "complete upload filed", nil).JSONBytes())
		return
	}
	totoalCount := 0
	chunkCount := 0
	for i := 0; i < len(data); i++ {
		...
		if k == "chunkcount" {
			totoalCount, _ = strconv.Atoi(v)
		} else if strings.HasPrefix(l, "chkidx_") && v == "1" {
			chunkCount++
		}
	}

	if totoalCount != chunkCount {
